"""Linux-only isolated network namespace owner for AutoNAT fixture scenarios.
...
class AutonatNetworkError(RuntimeError):
...
def _subprocess_command(command: list[str]) -> CommandResult:
    try:
        ...
        raise AutonatNetworkError(f"network command timed out: {' '.join(command)}") from error
    except OSError as error:
        raise AutonatNetworkError(f"could not start network command {' '.join(command)}: {error}") from error
    return CommandResult(completed.returncode, completed.stdout, completed.stderr)


class IsolatedAutonatNetwork:
    """A fresh outer netns containing only a bridge and AutoNAT fixture peers."""
    ...
    def __init__(
        ...
        *,
        ...
        self._namespaces = {"outer": f"autonat-o-{token}"}
        self._namespaces.update({"client": f"autonat-c-{token}", "server": f"autonat-s-{token}"})
    ...
    def namespace_command(self, role: str, command: Sequence[str | Path]) -> list[str]:
        ...
        if self._state != "ready":
            raise AutonatNetworkError("fixture network is not ready")
        if role not in self._roles:
            raise ValueError(f"unknown AutoNAT fixture role: {role}")
    ...
    def assert_closed_tcp_port(self, role: str, port: int) -> None:
        ...
        if not ss:
            raise AutonatNetworkError("closed-port assertion requires iproute2 'ss'")
        command = self.namespace_command(role, [ss, "-H", "-lnt", "sport", "=", f":{port}"])
        result = self._run_checked(command)
        if result.stdout.strip():
            raise AutonatNetworkError(f"negative control TCP port {port} is listening in {role}")
        self._closed_tcp_ports.append({"role": role, "port": port, "transport": "tcp", "listening": False})

    def setup(self) -> "IsolatedAutonatNetwork":
        if self._state == "ready":
            return self
        if self._state == "closed":
            raise AutonatNetworkError("closed fixture network cannot be reused")
        if self._state == "setting_up":
            raise AutonatNetworkError("fixture network setup is already in progress")
    ...
    def evidence(self) -> dict[str, object]:
        """Setup evidence only; this does not assert an AutoNAT outcome."""
        return {
            "kind": "linux_isolated_autonat_netns",
            "state": self._state,
    ...
    def __enter__(self) -> "IsolatedAutonatNetwork":
        return self.setup()
    ...
    def __exit__(self, exc_type, exc_value, traceback) -> bool:
        ...
        if exc_value is not None:
            ...
            return False
        raise AutonatNetworkError(f"fixture network cleanup failed: {'; '.join(errors)}")

    def _require_linux_iproute(self) -> None:
        system = self._system()
        if system != "Linux":
            raise AutonatNetworkError(f"isolated AutoNAT fixture network requires Linux, not {system}")
        self._ip = self._ip_lookup("ip")
        if not self._ip:
            raise AutonatNetworkError(
                "isolated AutoNAT fixture network requires iproute2 ('ip'); refusing to fall back to host networking"
            )
        if not self._outer_namespace_isolated():
            raise AutonatNetworkError(
                "isolated AutoNAT fixture network requires caller unshare --net --mount; refusing the shared host namespace"
            )
    ...
    def _verify_caller_outer_network(self) -> None:
        ...
        if not isinstance(routes, list) or routes:
            raise AutonatNetworkError("refusing non-isolated caller outer network: default route is present")
        ...
        if not isinstance(ipv6_routes, list) or ipv6_routes:
            raise AutonatNetworkError("refusing non-isolated caller outer network: IPv6 default route is present")
    ...
    def _validate_isolated_links(
        scope: str, links: list[dict[str, object]], expected_links: set[str]
    ) -> None:
    ...
    def _link_records(self, command: list[str], purpose: str) -> list[dict[str, object]]:
        ...
        if not isinstance(links, list) or any(
            ...
            raise AutonatNetworkError(f"invalid JSON from {purpose}: {' '.join(command)}")
        return links
    ...
    def _json_output(self, command: list[str], purpose: str) -> object:
        ...
        try:
            ...
            raise AutonatNetworkError(f"invalid JSON from {purpose}: {' '.join(command)}") from error

    def _add_namespace(self, role: str) -> None:
        assert self._ip is not None
        namespace = self._namespaces[role]
        if self._namespace_exists(namespace):
            raise AutonatNetworkError(f"fixture network namespace collision: {namespace} already exists")
        ...
        if result.returncode != 0:
            raise AutonatNetworkError(self._command_failure(command, result))
